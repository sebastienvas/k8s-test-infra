...
func TestExpectedStatus(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		state, desc := expectedStatus(queriesByRepo, &pr, pool, newContextRegister())
		if state != tc.state {
...
func TestPickBatch(t *testing.T) {
	...
	prs, err := c.pickBatch(sp, newContextRegister())
	if err != nil {
...
func TestTakeAction(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		if act, _, err := c.takeAction(sp, presubmits, batchPending, genPulls(tc.successes), genPulls(tc.pendings), genPulls(tc.nones), genPulls(tc.batchMerges), newContextRegister()); err != nil {
...
func TestIsPassing(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		pr := PullRequest{HeadRefOID: githubql.String(headSHA)}
		passing := isPassingTests(log, ghc, pr, rc)
