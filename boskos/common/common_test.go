...
func TestUserData_Update(t *testing.T) {
	ud1 := UserDataFromMap(map[string]string{"0": "0"})
	ud2 := UserDataFromMap(map[string]string{"1": "1", "2": "2"})
	ud3 := UserDataFromMap(map[string]string{"0": "0", "1": "1", "2": "2"})
	ud1.Update(ud2)
	if !reflect.DeepEqual(ud1.ToMap(), ud3.ToMap()) {
		t.Errorf("%v does not match expected %v", ud1, ud3)
	}
	// Testing delete
	ud3.Update(UserDataFromMap(map[string]string{"0": ""}))
...
func TestUserData_Marshall(t *testing.T) {
	ud := UserDataFromMap(map[string]string{"0": "0", "1": "1", "2": "2"})
	...
	if err := udFromJSON.UnmarshalJSON(b); err != nil {
		t.Errorf("uabble to unmarshall %v", string(b))
	}
...
func TestUserData_JSON(t *testing.T) {
	...
	if err := json.NewDecoder(b).Decode(&decodedUD); err != nil {
		t.Errorf("uabble to unmarshall %v", b.String())
	}
