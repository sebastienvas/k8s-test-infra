...
func UserDataFromMap(m map[string]string) UserData {
	ud := UserData{}
	for k, v := range m {
		ud.Store(k, v)
	}
...
// ToMap converts a UserData to map[string]string
func (ud UserData) ToMap() map[string]string {
	m := map[string]string{}
...
// ToMap converts a UserData to map[string]string
func (ud *UserData) FromMap(m map[string]string) {
